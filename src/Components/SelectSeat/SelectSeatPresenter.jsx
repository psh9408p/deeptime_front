...
const Sel = () => {
  ...
  const regionRenderer = regionProps => {
    if (!regionProps.isChanging) {
      return (
        <div style={{ position: "absolute", width: 0, bottom: 0, right: "-0.5em" }}>
  ...
  const SelectClass = video => {
    canvases[0].current
      .getContext("2d")
      .drawImage(video.current, 0, 0, video.current.videoWidth, video.current.videoHeight)
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex" }}>
        <div style={{ flex: 1 }}>
          <video playsInline width="320" height="240" autoPlay muted ref={videos[0]} />
        </div>
        <div style={{ flex: 1 }}>
          <video playsInline width="320" height="240" autoPlay muted ref={videos[1]} />
        </div>
      </div>
