...
const SchoolLevelDiv = styled.div`
  display: inline-flex;
  border: 0;
  border: ${(props) => props.theme.boxBorder};
  border-radius: ${(props) => props.theme.borderRadius};
  ...
  return (
    <SchoolRow>
      ...
      <HeaderColumn>
          ...
          {isSelf ? (
            <>
                ...
                {(close) => (
                  <PBody>
                    <form onSubmit={onSubmit}>
                      ...
                      <SchoolLevelDiv>
                        <span>분류 :</span>
                        <Select {...schoolLevel} id={"modifySchoolLevel"} />
                      </SchoolLevelDiv>
                      <ButtonDiv>
                        <PopupButton text={"수정"} />
                        <PopupButton
              ...
              <EditButton
                type="button"
                onClick={async () => {
                  toast.info(`'${school.name}'를 관리 목록에서 삭제 중...`)
                  await deleteSchoolMutation()
                  await schoolRefetch()
                  toast.success(`'${school.name}'가 관리 목록에서 삭제되었습니다.`)
                }}
                text="삭제"
              />
            </>
        ...
        {school.studentsCount !== undefined && (
          <Count>
            <FatText text={String(school.studentsCount)} /> 학생
          </Count>
        )}
        <Adress>
          <DetailAdress>
