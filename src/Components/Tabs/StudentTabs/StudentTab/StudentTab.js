...
export default ({
  ...
  const loadValue = async () => {
    ...
    if (loginPosition === "manager_school") {
      ...
      selectChange("modifySchool", mySchoolList.optionList.indexOf(`${student.schools[0].name}`))
      ...
      <HeaderColumn>
        <NameRow>
          ...
          <EditButton
            type="button"
            onClick={async () => {
              toast.info(`'${student.fullName}'(을)를 관리 목록에서 삭제 중...`)
              await deleteStudentMutation()
              await disconSeatMutation()
              await studentRefetch()
              toast.success(`'${student.fullName}'(이)가 관리 목록에서 삭제되었습니다.`)
            }}
            text="삭제"
          />
          <EditButton
        ...
        <Info>
          ...
          {loginPosition === "manager_academy" && (
            <DetailInfo>
              <FatText text="학원: " />
              {student.academies[0]?.name}
            </DetailInfo>
          )}
          {loginPosition === "manager_readingRoom" && (
            <DetailInfo>
              <FatText text="독서실: " />
              {student.readingRooms[0]?.name}
            </DetailInfo>
          )}
          <DetailInfo>
            <FatText text="클래스: " />
            {loginPosition === "manager_school" && student.classesOfSchool[0].name}
            {loginPosition === "manager_academy" && student.classesOfAcademy[0].name}
            {loginPosition === "manager_readingRoom" && student.classesOfReadingRoom[0].name}
          </DetailInfo>
          <DetailInfo>
            <FatText text="좌석: " />
            {loginPosition === "manager_school" &&
