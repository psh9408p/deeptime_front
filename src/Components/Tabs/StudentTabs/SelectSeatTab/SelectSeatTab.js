...
export default () => {
  ...
  const addCallback = async (row, number, id, addCb) => {
  ...
  const removeCallback = async (row, number, id, removeCb) => {
    await new Promise(resolve => setTimeout(resolve, 1000))
  ...
  const addSeatCallback = async ({ row, number, id }, addCb) => {
    await setSeatLoading(true)
    await addCallback(row, number, id, addCb)
    await setSeatLoading(false)
  }

  const removeSeatCallback = async ({ row, number, id }, removeCb) => {
    await setSeatLoading(true)
    await removeCallback(row, number, id, removeCb)
    await setSeatLoading(false)
  }
