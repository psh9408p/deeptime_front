...
export default function Attendance({ Mydata }) {
    ...
    if (faceButton) {
      ...
      facePredinctions.forEach((detection, i) => {
        ...
        if (faceResults[i].label === Mydata.me.username) {
          ...
          ctx.strokeText("모르는 얼굴...", detection.detection.box._x, detection.detection.box._y)
        }
      })
    }
  }
  ...
  const ConcludeFinaldecision = (
    ...
    if (Math.max(allfaceResults) === 1 && objectPredictions !== null && posePredictions !== null) {
      result = true
    } else {
      result = false
    }
  ...
  return (
    <div>
      ...
      <div>
        <video ref={video1} playsInline width="640" height="240" autoPlay muted />
